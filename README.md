...
更详细的子命令说明见 `files/CLI-COMMANDS.md`。
...
见 `files/NPM-PUBLISH.md`。

**全局安装（推荐给普通 GitHub 用户）：**
...
Logpilot 使用 **OpenAI 兼容 API**，默认提供方为 **OpenAI 兼容**，也可切换为 **DeepSeek**。
...
| 变量 | 说明 |
|------|------|
| `LOGPILOT_PROVIDER` | `openai`（默认）或 `deepseek` |
| `LOGPILOT_API_KEY` / `OPENAI_API_KEY` | OpenAI 兼容 API Key |
| `DEEPSEEK_API_KEY` | DeepSeek 专用 Key（可选；不填则用 `LOGPILOT_API_KEY`） |
| `LOGPILOT_API_BASE` | 自定义网关 Base URL（可选，用于代理 / 企业网关） |
| `LOGPILOT_MODEL` | 模型名（可选；openai 默认 `gpt-4o-mini`，deepseek 默认 `deepseek-chat`） |
...
```bash
cp .env.example .env
# 填写 LOGPILOT_PROVIDER / LOGPILOT_API_KEY 等（也兼容旧的 WORKLOG_*）
```
...
更完整、更详细的命令参数说明，请查看 `files/CLI-COMMANDS.md`。
...
更详细的 Prompt 设计可参考 `files/REPORT-PROMPT.md`。
