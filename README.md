...
workpilot week --provider openai
```
...
```bash
# 方式 A：仅本次命令生效（推荐临时使用）
OPEN_AI_API_KEY=sk-xxx AI_PROVIDER=openai workpilot day
...
DEEPSEEK_API_KEY=sk-xxx workpilot day
```
...
```bash
workpilot day --repo https://github.com/user/project
```
...
* **日常开发者**：每完成一块功能，使用 `workpilot commit` 生成规范的提交说明；下班前用 `workpilot day` 生成日报。
* **外企工程师**：用中文日报对内同步；通过 `--lang en` 附加英文版本对外或跨团队同步。
* **个人开源 / side project**：每周使用 `workpilot week`，快速回顾本周在项目上的投入与成果。
