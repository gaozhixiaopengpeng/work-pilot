...
Worklog 会自动读取你的 Git commit 和代码 diff，然后通过 AI 帮你生成：
...
Worklog 使用 **OpenAI 兼容 API**，默认提供方为 **OpenAI 兼容**，也可切换为 **DeepSeek**。
...
| `WORKLOG_API_BASE` | 自定义网关 Base URL（可选，用于代理 / 企业网关） |
...
```bash
cp .env.example .env
# 填写 WORKLOG_PROVIDER / WORKLOG_API_KEY 等
```
...
git add -A   # 先暂存变更
worklog commit
# 使用暂存区 diff：worklog commit --staged
...
* **日常开发者**：每完成一块功能，使用 `worklog commit` 生成规范的提交说明；下班前用 `worklog day` 生成日报。
* **外企工程师**：用中文日报对内同步；通过 `--lang en` 附加英文版本对外或跨团队同步。
* **个人开源 / side project**：每周使用 `worklog week`，快速回顾本周在项目上的投入与成果。
